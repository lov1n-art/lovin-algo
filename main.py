...
"""
Main script to run the long-only breakout strategy.
"""
...
def main():
    ...
    logging.info("Starting long-only breakout strategy monitoring...")
    ...
    while True:
        ...
        if current_time >= config.SQUARE_OFF_TIME:
            ...
            order_manager.square_off_all_positions()
            break
        ...
        for symbol in config.STOCKS:
            try:
                ...
                # Check for breakout
                resistance_level = config.RESISTANCE_LEVELS.get(symbol)
                is_breakout, breakout_idx = check_breakout(df, resistance_level)
