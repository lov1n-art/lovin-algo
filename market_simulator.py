...
class SimulatedKiteConnect:
    ...
    def _generate_simulated_data(self, instrument_token, from_date, to_date, interval):
        ...
        base_price = instrument_token % 1000 * 100
        
        # Generate prices with an upward bias for testing long breakouts
        prices = np.random.normal(loc=base_price, scale=base_price*0.01, size=len(dates))
        trend = np.linspace(0, 10, len(dates))  # Add upward trend
        prices = prices + trend
