# Short-Only Intraday Breakout Trading Strategy

## 1. Project Overview

This project is a Python-based script that implements a systematic, short-only trading strategy designed for intraday breakouts. The script is built to interface with the Zerodha Kite Connect API, allowing it to fetch historical data and (in a live environment) place trades automatically.
...
*   **Entry Trigger:** A short position is initiated when a 3-minute candle closes below a manually pre-defined support level for a stock.
*   **Stop-Loss Placement:** The stop-loss is automatically calculated and set to the **LOWER** of two values to ensure the tightest possible risk management:
    1.  The highest price reached in the two 3-minute candles immediately preceding the breakout candle.
    2.  2% above the entry price.
*   **Target:** The take-profit target is set at a 1:2 Risk/Reward ratio, meaning the target distance from the entry price is twice the stop-loss distance.
*   **Position Sizing:** The trade size is calculated based on a fixed risk amount per trade (e.g., ₹1,000), defined in the configuration file. The formula is: `Position Size = Risk Amount / Stop-Loss Distance`.
...
*   `config.py`: A user-configurable file to store all your settings, such as API credentials, the list of stocks to track, their support levels, and risk parameters.
...
2.  **Stocks and Support Levels:**
    *   Update the `STOCKS` list with the stock symbols you want to track (e.g., `"RELIANCE"`, `"TCS"`).
    *   Update the `SUPPORT_LEVELS` dictionary to map each stock symbol to its manually identified support level.
3.  **Risk Parameters:** Adjust `RISK_PER_TRADE` to your desired risk amount in your currency (e.g., `500` for ₹500).
